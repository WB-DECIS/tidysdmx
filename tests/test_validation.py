...
class TestValidateCodelistIds:
    ...
    def test_missing_values_are_not_codelist_violations(self, sample_codelist_ids):
        """NaN cells are not reported as codelist violations (API-06).

        Missing values are the missing-values check's job; a NaN must not be
        stringified into a spurious ``'nan'`` code here.
        """
        df = pd.DataFrame({"col1": ["A1", None], "col2": ["B1", "B2"]})
        # No ValueError, and in particular no 'nan' reported.
        validate_codelist_ids(df, sample_codelist_ids)

    def test_missing_value_alongside_real_violation(self, sample_codelist_ids):
        """A NaN is ignored while a genuine out-of-codelist value still fails."""
        df = pd.DataFrame({"col1": ["A1", None, "WRONG"], "col2": ["B1", "B2", "B1"]})
