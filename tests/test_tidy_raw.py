...
class TestFilterRows:
    ...
    def test_filter_rows_raises_on_invalid_df(self, invalid_df):
    ...
    def test_filter_rows_returns_dataframe_with_same_columns(self):
    ...
    def test_filter_rows_index_type_preserved(self):
        """Ensure index type is preserved in returned DataFrame."""
        df = pd.DataFrame(
            {"A": ["1", "2"]}, index=pd.Index([10, 20], name="custom_index")
        )
        codelist_ids = {"A": ["1"]}
        result = filter_rows(df, codelist_ids)
    ...
    def test_filter_rows_empty_result_has_correct_structure(self):
    ...
    def test_filter_rows_basic_filtering(self, sample_df, codelist_ids, expected_rows):
        result = filter_rows(sample_df, codelist_ids)
        assert list(result.index) == expected_rows

    def test_filter_rows_missing_column_handling(self, sample_df):
    ...
    def test_filter_rows_preserves_dtypes(self, sample_df):
        result = filter_rows(sample_df, {"code": [1, 2]})
        assert str(result["code"].dtype) == "int64"

    def test_filter_rows_does_not_mutate_input(self, sample_df):
    ...
    def test_filter_rows_nan_values_are_not_dropped(self, sample_df):
    ...
    def test_filter_rows_allowed_values_as_strings(self, sample_df):
...
class TestFilterTidyRaw:
    def test_filter_tidy_raw_returns_dataframe(self, sdmx_schema, sdmx_df):
    ...
    def test_filter_tidy_raw_empty_dataframe(self, sdmx_schema):
    ...
    def test_filter_tidy_raw_raises_on_invalid_inputs(self, invalid_df, invalid_schema):
